...
func (self *BinStorageWrapper) bootStrapRing() {
	var cli *client
	if LogEnabled { log.Print("Entered the ring") }
...
//	var name string
	...
	if LogEnabled { log.Print("Entering updateRing") }
	...
	for {
		...
		if LogEnabled { log.Print(len(self.chord.ring)) }
		for i:= range self.chord.ring{
	...
	self.printRing()
	if LogEnabled { log.Print("Leaving updateRing") }
}
